...
window.qyImport(function (lib, game, ui, get, ai, _status, config) {
    ...
    // 拖拽读取文件
    // 官方文档：https://www.electronjs.org/zh/docs/latest/api/file-object
    document.addEventListener('drop', (e) => {
        ...
        window.qyCachesMainWindow.postMessage({
            ...
            data: {
                ...
                query: {
                    zipPath: file.path,
                    node7Zip: window.require(`${window.__dirname}/extension/假装无敌/node7zip-bin`)['path7za'],
                    isDebug: false
                },
            }
        });
    ...
    if (config.qingyao_shoushatexiao) {
        //此代码已经完全独立重写
        lib.skill._qyshousha_jisha = {
            ...
            content: function () {
                if (player.storage._qyshousha_jisha2 == 1) {
                    ...
                    JzwdPlayAnimDelay ? JzwdPlayAnimDelay('qy_yipo', 2500, {scale: 0.7}) : current.$skill('一破·卧龙出山', 'fire', 'fire', 'avatar');
                    game.playqysstx('qyjstx_jisha1');
                    //_status.qyshousha_jisha=true;
                    //}
                }
                if (player.storage._qyshousha_jisha2 == 2) {
                    JzwdPlayAnimDelay ? JzwdPlayAnimDelay('qy_shuanglian', 2500, {scale: 0.7}) : current.$skill('双连·一战成名', 'fire', 'fire', 'avatar');
                    game.playqysstx('qyjstx_jisha2');
                }
                if (player.storage._qyshousha_jisha2 == 3) {
                    JzwdPlayAnimDelay ? JzwdPlayAnimDelay('qy_sanlian', 2500, {scale: 0.7}) : current.$skill('三连·举世皆惊', 'fire', 'fire', 'avatar');
                    game.playqysstx('qyjstx_jisha3');
                }
                if (player.storage._qyshousha_jisha2 == 4) {
                    JzwdPlayAnimDelay ? JzwdPlayAnimDelay('qy_silian', 2500, {scale: 0.7}) : current.$skill('四连·天下无敌', 'fire', 'fire', 'avatar');
                    game.playqysstx('qyjstx_jisha4');
                }
                if (player.storage._qyshousha_jisha2 == 5) {
                    JzwdPlayAnimDelay ? JzwdPlayAnimDelay('qy_wulian', 2500, {scale: 0.7}) : current.$skill('六连·诛天灭地', 'fire', 'fire', 'avatar');
                    game.playqysstx('qyjstx_jisha5');
                }
                if (player.storage._qyshousha_jisha2 == 6) {
                    JzwdPlayAnimDelay ? JzwdPlayAnimDelay('qy_liulian', 2500, {scale: 0.7}) : current.$skill('六连·诛天灭地', 'fire', 'fire', 'avatar');
                    game.playqysstx('qyjstx_jisha6');
                }
                if (player.storage._qyshousha_jisha2 >= 7) {
                    JzwdPlayAnimDelay ? JzwdPlayAnimDelay('qy_qilian', 2500, {scale: 0.7}) : current.$skill('七连·诛天灭地', 'fire', 'fire', 'avatar');
                ...
                if(trigger.num>0){
                    ...
                    if(trigger.num==-3){
                        game.broadcastAll(function () {
                            JzwdPlayAnimDelay ? JzwdPlayAnimDelay("qy_diankuangtulu", 2500, {scale: 0.6}) : player.$skill('癫狂屠戮', 'fire', 'fire', 'avatar');
                            game.playqysstx('qy_diankuangtulu');
                        });
                    }
                    else{
                        game.broadcastAll(function () {
                            JzwdPlayAnimDelay ? JzwdPlayAnimDelay("qy_wushuangwanjunqushou", 2500, {scale: 0.6}) : player.$skill('无双·万军取首', 'fire', 'fire', 'avatar');
    ...
    if (config.qingyao_shoushapeiyin) {
        if (lib.skill.qilin_skill) lib.skill.qilin_skill.audio = "ext:假装无敌:true";
        if (lib.skill.qibaodao2) lib.skill.qibaodao2.audio = "ext:假装无敌:true";
        if (lib.skill.lanyinjia) lib.skill.lanyinjia.audio = "ext:假装无敌:true";
        if (lib.skill.cixiong_skill) lib.skill.cixiong_skill.audio = "ext:假装无敌:true";
        ...
        lib.skill['_qy-replace-card'] = {
            ...
            content: function () {
                ...
                if (result.bool) {
                    ...
                    dialog.style.setProperty('background-image', 'url("' + lib.assetURL + 'extension/假装无敌/images/replace_card_backronud.png")', 'important');
                    dialog.content.css({
                        'margin-top': '2%',
                    });
                    dialog.addEventListener(lib.config.touchscreen ? 'touchend' : 'mouseup', function () {
    ...
    get.qyRateNum = function (name) {
        ...
        let num = 1;
        const rateCharacter = game.getExtensionConfig('假装无敌', 'rateCharacter') || {};
    ...
    if (config.qingyao_shoushaMVP) {
        ...
        _status.手杀MVP = function (sd, isDragLocation) {
            ...
            setTimeout(item => {
                ...
                // 静态MVP, 静态版本不再维护,请注意！
                if (!lib.config.extension_假装无敌_qingyao_toggle_MVP) {
                    let config = {
                        scale: 1, x: 390, loop: true, y: 310
                    };
                    if (lib.config.touchscreen) {
                        config = {
                            scale: 1.05, x: 430, loop: true, y: 294
                        }
                    }

                    JzwdPlayAnimDelay(['qy_mvp', 'animation'], Object.assign(config, {
                    ...
                    var avatar = ui.create.div('.qy-avatar-1', popuperContainer);
                    ...
                    if (game.getExtensionConfig("假装无敌", "qingyao_outcrop")) {
                        ...
                        avatar.css({
                            height: isMobeil ? '59vh' : '294px',
                            top: isMobeil ? '17.6vh' : '86px',

                        })
                    }
                    var avatarBorder = ui.create.div('.qy-avatar-border-1', popuperContainer);
                    avatarBorder.dataset.name = get.translation(player.name);
                    avatarBorder.setBackgroundImage(`extension/假装无敌/images/border_${player.group}.png`);
                } else {
                    var config = [{
                    ...
                    if (game.getExtensionConfig('假装无敌', 'qingyaoDynamicMvpEnable')) {
                        let qingyao_dynamic_x = game.getExtensionConfig('假装无敌', 'qingyao_dynamic_x');
                        if (qingyao_dynamic_x !== 0 && isFinite(qingyao_dynamic_x)) {
                            config[0].x += qingyao_dynamic_x;
                        }
                        let qingyao_dynamic_y = game.getExtensionConfig('假装无敌', 'qingyao_dynamic_y');
                    ...
                    JzwdPlayAnimDelay('qy_quanchangzuijia', Object.assign(config[0], {
                        complete: function (track, position) {
                            if (!position.isComplete) {
                                position.isComplete = true;
                    ...
                    ui.create.div('.qy-mvp-share-button', popuperContainer).addEventListener('click', event => {
                        ...
                        if (window.cordova) {
                            var success = function (message) {
                                console.log("掉用成功！", message);
                            }, fail = function (message) {
                                message = ("分享失败：" + (message || "请使用清瑶版进行分享"));
                                alert(message);
                            }
                            window.cordova.exec(success, fail, 'PretendInvincible', 'shareScreenShot', []);
                        } else {
                            alert("电脑版暂不支持分享功能，手机请使用清瑶版！")
                        }
                    });
                ...
                // 公共代码部分
                var image = new Image();
                image.src = `${lib.assetURL}extension/假装无敌/images/border_${player.group}.png`;
                image.onerror = function () {
                    avatarBorder.setBackgroundImage(`extension/假装无敌/images/border_qun.png`);
                }
                ...
                var icon = ui.create.div('.qy-mvp-player-icon', {
                    height: '80px',
                }, rightContainer);
                var qyIconImage = game.getExtensionConfig('假装无敌', 'qyIconImage');
                if (qyIconImage)
                    icon.css({
                ...
                if (game.me === player) ui.create.node('img', nickname).src = lib.assetURL + 'extension/假装无敌/images/mvp_me_tag.png';
                ...
                const zoomClassList = ['.qy-avatar', '.qy-avatar-1', '.qy-avatar-border-1', '.qy-avatar-border', '.qy-mvp-player-icon'];
                if (isDragLocation){
                    (function dragMvp() {
                        // 声明
                        ...
                        InitializeSettings.addEventListener('click', event => {
                            dragClassList.forEach(item=> game.saveExtensionConfig('假装无敌', item));
                            zoomClassList.map(item=>`${item}_zoom`).forEach(item=> game.saveExtensionConfig('假装无敌', item));
                            click(event, true);
                        });
                    })()
                }
                // 移动位置
                dragClassList.map(item => [document.querySelector(item)].concat(game.getExtensionConfig('假装无敌', item) || [null, null]))
                ...
                zoomClassList.map(item => [document.querySelector(item)].concat(game.getExtensionConfig('假装无敌', `${item}_zoom`) || [null, null]))
                    .filter(arrayElement => arrayElement[0] != null)
                    .forEach(arrayElement => {
                        let element = arrayElement[0];
    ...
    lib.skill['_qy-mvp-general-opening'] = {
        ...
        popup: false,
        filter: function () {
            return game.getExtensionConfig('假装无敌', 'qingyao_general_opening') && arguments[1] === game.me;
        },
        content: function () {
            setTimeout(function () {
            ...
            if (window.qy_decadeUI && window.qy_decadeUI.Animation) {
                Object.assign(window, {
                    JzwdAnimation: (function () {
                        var animation = new qy_decadeUI.Animation('extension/假装无敌/animation/', document.body, 'Jzwd-canvas-animation');
                        animation.check();
                        var fileInfoList = [
                            // 大将军开场特效
                    ...
                    JzwdPlayAnimDelay: function (name, time, position) {
                        ...
                        if(window.isMobile){
                            let canvas = JzwdAnimation.canvas;
                            canvas.width = ui.window.clientWidth;
                            canvas.height = ui.window.clientHeight;
                        }
    ...
    lib.skill['_qy_tiesuo_suo'] = {
        ...
        filter: function (event, player) {
            var tiesuoConfig = game.getExtensionConfig('假装无敌', 'qingyao_tiesuo');
    ...
    lib.arenaReady.unshift(item => {
        ...
        const limitSkillAnimation = function (player, type, font) {
            ...
            const {
                extension_假装无敌_qingyao_limited_x,
                extension_假装无敌_qingyao_limited_y,
                extension_假装无敌_qingyao_limited_back_x,
                extension_假装无敌_qingyao_limited_back_y,
                extension_假装无敌_qingyao_awaken_x,
                extension_假装无敌_qingyao_awaken_y,
                extension_假装无敌_qingyao_awaken_back_x,
                extension_假装无敌_qingyao_awaken_back_y,
                extension_假装无敌_qingyao_limited_enable,
                extension_假装无敌_qingyao_awaken_enable
            } = lib.config;
            let x = 0, y = 0;
            if (['qy_juexingji', 'qy_shimingji'].contains(type)) {
                if (extension_假装无敌_qingyao_awaken_enable) {
                    if (isFinite(extension_假装无敌_qingyao_awaken_x)) x = extension_假装无敌_qingyao_awaken_x;
                    if (isFinite(extension_假装无敌_qingyao_awaken_y)) y = extension_假装无敌_qingyao_awaken_y;
                    if (isFinite(extension_假装无敌_qingyao_awaken_back_x)) backConfig.x += extension_假装无敌_qingyao_awaken_back_x;
                    if (isFinite(extension_假装无敌_qingyao_awaken_back_y) && extension_假装无敌_qingyao_awaken_back_y != 0) backConfig.y = extension_假装无敌_qingyao_awaken_back_y;
                }
            } else {
                if (extension_假装无敌_qingyao_limited_enable) {
                    if (isFinite(extension_假装无敌_qingyao_limited_x)) x = extension_假装无敌_qingyao_limited_x;
                    if (isFinite(extension_假装无敌_qingyao_limited_y)) y = extension_假装无敌_qingyao_limited_y;
                    if (isFinite(extension_假装无敌_qingyao_limited_back_x)) backConfig.x += extension_假装无敌_qingyao_limited_back_x;
                    if (isFinite(extension_假装无敌_qingyao_limited_back_y) && extension_假装无敌_qingyao_limited_back_y != 0) backConfig.y = extension_假装无敌_qingyao_limited_back_y;
                }
            }
    ...
    if (game.getExtensionConfig('假装无敌', 'resizeLeftFontPosition') !== void 0) {
        ...
        window.resizeLeftFontPosition.innerHTML = `
                  .qy-mvp-avatarborder::before{
                      left: ${game.getExtensionConfig('假装无敌', 'resizeLeftFontPosition')}% !important;
                  }`;
    }
    if (game.getExtensionConfig('假装无敌', 'resizeRightFontPosition') !== void 0) {
        ...
        window.resizeRightFontPosition.innerHTML = `
                .qy-mvp-scoreInfo{
                    left: ${game.getExtensionConfig('假装无敌', 'resizeRightFontPosition')}vw;
                }`;
    }
    ...
    if (game.getExtensionConfig('假装无敌', 'rePositionXing') !== void 0) {
        ...
        window.rePositionXing.innerHTML = `
                .qy-mvp-xing {
                    left: ${game.getExtensionConfig('假装无敌', 'rePositionXing')}%;
                }`;
    }
    ...
    if (game.getExtensionConfig('假装无敌', 'qingyaoDynamicMvpEnable')) {
        ...
        if (game.getExtensionConfig('假装无敌', 'resizeDynamicLeftXing') !== void 0) {
            ...
            .qy-mvp-border-xing{
                left: ${game.getExtensionConfig('假装无敌', 'resizeDynamicLeftXing')}px;
            }`;
        }
    }
            ...
            content: function () {
                ...
                setTimeout(function () {
                    ...
                    history.arena.css({
                        ...
                        left: 0,
                        backgroundSize: '100% 100%',
                        backgroundImage: `url(${lib.assetURL}extension/假装无敌/images/back.jpeg)`,
                    })
                }, 200);
...
window.qyCachesMainWindow = {
    src: 'extension/假装无敌/dist/index.html?openRouter=',
    mainWindow: null,
    /**
     * @param index 0:包裹的div,1:iframe,2:iframe.contentWindow
     * @returns {*}
     */
    getMain(index = 0) {
...
setTimeout(function () {
    ...
    /*downloadFile[3].postMessage({
        type: 'methods',
        data: 'checkUpdateForBody',
    });*/
}, 200);
